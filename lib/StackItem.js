...
type Props = {
  ...
  closeModal: (stackItem: StackItemType, onClose: ?() => void) => void,
...
class StackItem extends Component<Props> {
  ...
  _updateAnimatedValue = (
    ...
    if (!shouldAnimateOut) closeModal?.(stackItem, onClose)
    ...
    Animated.timing(this.animatedValue, {
      ...
      if (finished && closeModal && shouldAnimateOut)
        closeModal(stackItem, onClose)
    })
  }
  ...
  _getAnimatedComponent = (
    ...
    const onAnimatedClose = (stackItem: string, onClose: ?() => void) =>
      this._updateAnimatedValue(position - 1, closeModal, onClose)
